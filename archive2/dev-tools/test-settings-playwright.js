...
const ADMIN_URL = process.env.WP_ADMIN_URL || 'http://hssb.test/wp-admin/';
const ADMIN_PAGE = process.env.WP_ADMIN_PAGE || 'admin.php?page=html-social-share';
const REACT_ADMIN_PAGE = process.env.WP_REACT_ADMIN_PAGE || 'admin.php?page=html-social-share-react';
...
async function testSettingsSave() {
    ...
    try {
        ...
        // Additional test: Check React Admin Interface
        console.log('Testing React Admin Interface...');
        try {
            await page.goto(`${ADMIN_URL}${REACT_ADMIN_PAGE}`, {
                waitUntil: 'networkidle'
            });

            // Wait for React app to mount
            console.log('Waiting for React app to mount...');
            await page.waitForSelector('#hss-admin-react-root', { timeout: 10000 });

            // Check if React app loaded
            const reactContent = await page.textContent('#hss-admin-react-root');
            if (reactContent && reactContent.includes('Social Share')) {
                console.log('✅ React Admin Interface: Loaded successfully');
            } else {
                console.log('⚠️  React Admin Interface: May still be loading or not built');
            }

            // Take screenshot of React interface
            await page.screenshot({ path: 'react-admin-interface.png' });
            console.log('Screenshot taken: react-admin-interface.png');

        } catch (e) {
            console.log('⚠️  React Admin Interface test failed:', e.message);
            console.log('This is expected if React app is not yet built');
        }
