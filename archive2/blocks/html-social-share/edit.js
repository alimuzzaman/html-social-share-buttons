...
export default function Edit( { attributes, setAttributes } ) {
		...
		if ( enabledNetworks.length === 0 ) {
			return (
				<div className="social-share-preview-placeholder">
					<p>
						{ __(
							'Select networks to display preview',
							'html-social-share'
						) }
					</p>
				</div>
	...
	return (
		<>
			...
			<InspectorControls>
				<PanelBody
					title={ __( 'Profile Settings', 'html-social-share' ) }
					initialOpen={ true }
				>
					<SelectControl
						...
						help={ __(
							'Choose a predefined profile or create a custom configuration.',
							'html-social-share'
						) }
					/>
					...
					{ isLoadingProfiles && (
						<p>
							{ __( 'Loading profiles…', 'html-social-share' ) }
						</p>
					) }
				</PanelBody>

				<PanelBody
					title={ __( 'Networks', 'html-social-share' ) }
				...
				>
					...
					{ availableNetworks.map( ( network ) => (
						<CheckboxControl
							...
							onChange={ ( enabled ) =>
								updateNetworkSetting( network.id, enabled )
							}
						/>
					) ) }
				</PanelBody>
				...
				>
					<SelectControl
						...
						options={ [
							{
								label: __( 'Default', 'html-social-share' ),
								value: 'default',
							},
							{
								label: __( 'Rounded', 'html-social-share' ),
								value: 'rounded',
							},
							{
								label: __( 'Square', 'html-social-share' ),
								value: 'square',
							},
							{
								label: __( 'Minimal', 'html-social-share' ),
								value: 'minimal',
							},
						] }
						onChange={ ( value ) =>
					...
					<SelectControl
						...
						options={ [
							{
								label: __( 'Small', 'html-social-share' ),
								value: 'small',
							},
							{
								label: __( 'Medium', 'html-social-share' ),
								value: 'medium',
							},
							{
								label: __( 'Large', 'html-social-share' ),
								value: 'large',
							},
						] }
						onChange={ ( value ) =>
					...
					<ToggleControl
						...
						help={ __(
							'Display network names alongside icons.',
							'html-social-share'
						) }
					/>
					...
					<ToggleControl
						...
						help={ __(
							'Display only icons without background styling.',
							'html-social-share'
						) }
					/>
				</PanelBody>
				...
				<PanelBody
					title={ __( 'Custom Share Content', 'html-social-share' ) }
					initialOpen={ false }
				>
					<TextControl
					...
					<TextControl
						...
						help={ __(
							'Leave empty to use the current page title.',
							'html-social-share'
						) }
					/>
					...
					<TextControl
						...
						help={ __(
							'Leave empty to use the current page excerpt.',
							'html-social-share'
						) }
					/>
				</PanelBody>
				...
				<PanelBody
					title={ __( 'Advanced', 'html-social-share' ) }
					initialOpen={ false }
				>
					<TextControl
				...
				<div className="social-share-block-wrapper">
					<div className="social-share-block-header">
						...
						<span className="social-share-block-subtitle">
							{ profileId
								? __( 'Profile:', 'html-social-share' ) +
								  ( profiles.find( ( p ) => p.id === profileId )
										?.name ||
										__( 'Unknown', 'html-social-share' ) )
								: __(
										'Custom Configuration',
										'html-social-share'
								  ) }
