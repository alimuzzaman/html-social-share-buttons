...
final class LegacySettingsMapper implements SettingsCodec {
	...
	public function fromArray( array $stored ) {
		...
		return new Settings(
			...
			$this->normalizeNetworkStates( $icons ),
	...
	public function toArray( Settings $settings, array $original = array() ) {
		...
		$original['icons']            = $this->toLegacyNetworkStates(
	...
	private function normalizeNetworkStates( array $stored ) {
		$states = array();
	...
	private function toLegacyNetworkStates( array $states, array $original ) {
		...
		foreach ( $states as $networkId => $enabled ) {
			if (
				...
				if ( $enabled ) {
					$legacy['twitter'] = $this->toBoolean( $original['twitter'] )
						? $original['twitter']
						: '1';
				} else {
					unset( $legacy['twitter'] );
				}
				continue;
			}
			...
			if ( $enabled ) {
				$legacy[ $networkId ] =
					array_key_exists( $networkId, $original ) &&
					$this->toBoolean( $original[ $networkId ] )
						? $original[ $networkId ]
						: '1';
			} elseif (
				! array_key_exists( $networkId, $original ) ||
	...
	private function toBoolean( $value ) {
		return LegacyTruthiness::isTruthy( $value );
	}
