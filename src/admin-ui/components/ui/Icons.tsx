...
export const AdminIcon: React.FC< AdminIconProps > = ( {
	...
	const [ WpIcon, setWpIcon ] = useState< React.ComponentType< any > | null >(
		null
	);
	...
	if ( candidates.includes( 'trash' ) ) {
		return <TrashIcon size={ size } className={ className } />;
	}
	if ( candidates.includes( 'refresh' ) || candidates.includes( 'update' ) ) {
		return <RefreshIcon size={ size } className={ className } />;
	}
