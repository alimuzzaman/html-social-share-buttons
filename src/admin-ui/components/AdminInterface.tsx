...
export const AdminInterface: React.FC = () => {
	...
	const { shareCounts, loading, error, refreshShareCounts } =
	...
	const handleBulkRefresh = async () => {
		if ( selectedPosts.length === 0 ) {
			setNotification( {
				type: 'error',
				message: __(
					'Please select posts to refresh',
		...
		try {
			...
			setNotification( {
				...
				message: sprintf(
					...
					_n(
						'Successfully refreshed %d post',
						'Successfully refreshed %d posts',
						result.processed,
						'html-social-share-buttons'
					),
					result.processed
			...
			setNotification( {
				type: 'error',
				message: __(
					'Failed to refresh share counts',
					'html-social-share-buttons'
				),
			} );
		} finally {
		...
		<div className="hss-admin-interface max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
			<div className="mb-8">
				<h1 className="text-3xl font-bold text-gray-900 mb-2">
					{ __(
						'Social Share Counts Manager',
						'html-social-share-buttons'
					) }
				</h1>
				<p className="text-gray-600 text-lg">
					{ __(
						'Manage and refresh social share counts across your site.',
						'html-social-share-buttons'
					) }
				</p>
			</div>
			...
			<div className="space-y-6">
				...
				<section className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
					<h2 className="text-lg font-medium text-gray-800 mb-3">
						{ __( 'Statistics', 'html-social-share-buttons' ) }
					</h2>
					<div className="hss-stats space-y-2">
						<p>
							<strong>
								{ __(
									'Total Posts with Counts:',
									'html-social-share-buttons'
								) }
							</strong>{ ' ' }
