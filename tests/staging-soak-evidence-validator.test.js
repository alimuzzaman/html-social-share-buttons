...
const { BASELINE, validateEvidence, validateRecord } = require('../scripts/validate-staging-soak-evidence');
...
function expectFailure(candidate, day, date, pattern) {
	assert.match(validateRecord(candidate, day, date, new Date('2026-08-30T00:00:00.000Z')).join('\n'), pattern);
}

const dayOne = record(1, '2026-08-12T09:04:40.000Z');
assert.deepStrictEqual(validateRecord(dayOne, 1, '2026-08-12', new Date('2026-08-13T00:00:00.000Z')), []);

const dayTwo = record(2, '2026-08-13T09:15:00.000Z');
assert.deepStrictEqual(validateRecord(dayTwo, 2, '2026-08-13', new Date('2026-08-14T00:00:00.000Z')), []);
...
assert.deepStrictEqual(validateRecord(dayFourteen, 14, '2026-08-25', new Date('2026-08-26T00:00:00.000Z')), []);
...
assert.deepStrictEqual(validateRecord(finalRollback, 14, '2026-08-26', new Date('2026-08-27T00:00:00.000Z'), true), []);
...
assert.match(validateRecord(earlyFinalRollback, 14, '2026-08-26', new Date('2026-08-27T00:00:00.000Z'), true).join('\n'), /completion threshold/);
...
assert.deepStrictEqual(validateRecord(changedBody, 2, '2026-08-13', new Date('2026-08-14T00:00:00.000Z')), []);
...
try {
	...
	writeRecord(dayOne);
	assert.deepStrictEqual(validateEvidence(temporaryRoot, new Date('2026-08-13T00:00:00.000Z'), 1).errors, []);
	assert.match(validateEvidence(temporaryRoot, new Date('2026-08-13T00:00:00.000Z'), 2).errors.join('\n'), /missing records through Day 2/);
	fs.writeFileSync(path.join(temporaryRoot, '2026-08-12', 'unexpected.txt'), 'unexpected');
	assert.match(validateEvidence(temporaryRoot, new Date('2026-08-13T00:00:00.000Z')).errors.join('\n'), /unrecognized/);
	...
	assert.match(validateEvidence(temporaryRoot, new Date('2026-08-14T00:00:00.000Z')).errors.join('\n'), /matching JSON sidecar is missing/);
} finally {
...
try {
	...
	assert.match(
		validateEvidence(completionRoot, new Date('2026-08-27T00:00:00.000Z'), 14, true).errors.join('\n'),
	...
	assert.deepStrictEqual(validateEvidence(completionRoot, new Date('2026-08-27T00:00:00.000Z'), 14, true).errors, []);
} finally {
