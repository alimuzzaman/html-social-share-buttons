...
const SHARE_SELECTOR = process.env.SHARE_SELECTOR || '.hss-share-buttons, .html-social-share-buttons, .hss-share';
...
async function testFrontendShareButtons() {
    ...
    try {
        ...
        console.log('Checking for share buttons using selector:', SHARE_SELECTOR);
        const buttons = await page.$$(SHARE_SELECTOR);

        if (!buttons || buttons.length === 0) {
            console.warn('No share button elements found. Selector may be incorrect or plugin not active on the homepage.');
        } else {
            ...
            try {
                ...
                if (firstAnchor) {
                    console.log('Found anchor inside share buttons, attempting click that opens share dialog');
                    ...
                    console.log('No anchor found inside first share button block');
                }
            } catch (e) {
                console.log('Error while interacting with share buttons:', e.message);
            }
        }
...
if (require.main === module) {
    testFrontendShareButtons().catch(err => { console.error(err); process.exit(1); });
}
