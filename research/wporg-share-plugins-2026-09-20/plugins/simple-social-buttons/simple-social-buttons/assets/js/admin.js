...
(function ($, window, document) {
	...
	$(
		function () {
			...
			$( '#ssb_popup\\[trigger_after_scrolling\\]' ).on(
				'change',
				function (_event) {
					if ($( this ).is( ':checked' )) {
			...
			$( document ).on(
				...
				function ( event ) {
					// Let React handle the button inside the Help page root (avoids double download).
					if ( $( event.target ).closest( '#ssb-help-root' ).length ) {
						return;
					}
			...
			$( "#ssb_click_to_tweet-tab-content form" ).on(
				'change',
				function (_e) {
					initCTT()
				}
			...
			function initCTT() {
				...
				$( '.ssb-ctt-wrapper > a' ).attr( 'data-href', 'https://twitter.com/intent/tweet?text=' + encodeURIComponent( tweetText ) )
			...
			$( document ).on(
				...
				function (e) {
					...
					$.ajax(
						...
						function (_jqXHR, _textStatus, _errorThrown) {
